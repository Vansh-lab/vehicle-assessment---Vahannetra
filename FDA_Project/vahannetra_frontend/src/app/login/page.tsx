  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-md items-center px-4 py-6">
      <Card className="w-full space-y-6 border-cyan-400/25">
        ...
        <form className="space-y-4" onSubmit={handleSubmit(() => router.push("/dashboard"))}>
          <div>
          ...
          <div>
            <Label htmlFor="otp" className="flex items-center gap-1"><Smartphone size={14} /> OTP / 2FA (optional)</Label>
            <Input id="otp" placeholder="123456" {...register("otp")} />
          </div>
          <Button className="w-full" type="submit"><ShieldCheck size={16} className="mr-2" /> Login</Button>
        </form>

        <div className="flex items-center justify-between text-xs text-slate-400">
          <Link href="#" className="hover:text-cyan-100">Forgot password?</Link>
          <span>SSO ready</span>
        </div>
      </Card>
    </div>
