...
from app.db_models import User, WebhookDeadLetter, WebhookSubscription
...
async def v1_list_webhook_dlq(
    ...
    query = (
        ...
        .where(WebhookDeadLetter.organization_id == current_user.organization_id)
        ...
        "items": [
            {
                ...
                "created_at": isoformat_utc_z(item.created_at),
                "resolved_at": (
                    isoformat_utc_z(item.resolved_at) if item.resolved_at else None
                ),
            }
            for item in items
        ]
...
async def v1_resolve_webhook_dlq(
    event_id: int,
    db: AsyncSession = Depends(get_async_db),
    current_user: User = Depends(get_current_user_async),
):
...
@router.post("/dlq/{event_id}/requeue")
async def v1_requeue_webhook_dlq(
    event_id: int,
    db: AsyncSession = Depends(get_async_db),
    current_user: User = Depends(get_current_user_async),
):
    ...
    item.resolved_at = utc_now()
    await db.commit()
    return {"id": item.id, "status": item.status, "task_id": task.id}
